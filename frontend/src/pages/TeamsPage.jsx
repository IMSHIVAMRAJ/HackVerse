...
const TeamsPage = () => {
    ...
    const fetchTeams = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:5000/api/requirements/team",
          {
            method: "GET",
            headers,
          }
    ...
    if (!searchTerm) {
      ...
      const filtered = teams.filter(
        (team) =>
          ...
          team.domains?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          team.message?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          team.skillNeeded?.toLowerCase().includes(searchTerm.toLowerCase())
      );
  ...
  if (loading) {
    return (
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, #667EEA 0%, #705EBD 50%, #764CA4 100%)",
        }}
      >
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-8 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-white/90 font-medium mb-4">
            Connect with amazing teams looking for talented members
          </p>
      ...
      {/* Teams Section - Matching the theme from the image */}
      <div className="bg-gray-50 min-h-screen">
        <div className="container mx-auto px-8 py-20">
          <div className="flex items-center justify-between mb-12">
            <h2 className="text-4xl font-bold text-center text-gray-900 w-full">
              Discover Teams
            </h2>
          </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-400">
                {searchTerm
                  ...
                  : "Check back later for new team opportunities!"}
              </p>
              {searchTerm && (
                <button
...
// Team Card Component - Styled to match the hackathon cards from the image
const TeamCard = ({ team }) => {
  const handleLinkedInClick = () => {
    if (team.linkdeinProfile) {
      window.open(team.linkdeinProfile, "_blank");
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border-0">
      <div className="p-6">
        {/* Header with status and time left - matching the hackathon card style */}
        ...
        {/* Team Name - matching the hackathon title style */}
        <h3 className="text-xl font-bold text-gray-900 mb-2">
          {team.teamname || "Team Names"}
        </h3>

        {/* Message - matching the hackathon description style */}
        <p className="text-gray-600 mb-6 text-sm leading-relaxed">
          {team.message || "No message provided"}
        </p>
        ...
        {team.domains && (
          <div className="mb-4">
            <div className="flex flex-wrap gap-2">
              {team.domains.split(",").map((domain, index) => (
                <span
                  key={index}
        ...
        {team.skillNeeded && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {team.skillNeeded.split(",").map((skill, index) => (
                <span
                  key={index}
        ...
        {/* Bottom section - matching the hackathon card bottom style */}
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center text-gray-600">
            <Users className="w-4 h-4 mr-2 text-blue-600" />
            <span className="text-sm font-medium">
              {team.currentMembers || 0} current • Need{" "}
              {team.requiredMembers || 0} more
            </span>
          </div>
        </div>
        ...
        <div className="border-t border-gray-200 pt-4">
          ...
          {!team.linkdeinProfile && !team.email && (
