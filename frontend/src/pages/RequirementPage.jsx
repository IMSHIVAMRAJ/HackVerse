...
const RequirementsPage = () => {
  ...
  // fetchRequirements extracted so it can be called from modals onClose
  const fetchRequirements = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/requirements/my`,
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch requirements: ${response.status} ${response.statusText}`
        );
      }
  ...
  const handleUpdateSuccess = (updatedRequirement) => {
    // merge updated item into list
    setRequirements((prev) =>
      prev.map((req) =>
        (req._id || req.id) === (updatedRequirement._id || updatedRequirement.id)
          ? updatedRequirement
          : req
      )
    );
  ...
  const handleDeleteSuccess = (deletedId) => {
    setRequirements((prev) => prev.filter((r) => (r._id || r.id) !== deletedId));
    setIsUpdateModalOpen(false);
    setSelectedRequirement(null);
  };

  // When create modal closes, refresh the list
  const handleCreateClose = () => {
    setIsCreateModalOpen(false);
    // refetch to pick up new requirement
    fetchRequirements();
  };
  ...
  if (loading) {
    return (
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #667EEA 0%, #705EBD 50%, #764CA4 100%)",
        }}
      >
      ...
      <div className="container mx-auto px-8 py-16">
        <div className="flex items-center justify-between mb-12">
          <div className="text-left">
            <h1 className="text-5xl lg:text-6xl font-bold text-white mb-2">My Requirements</h1>
            <p className="text-xl text-white/90 font-medium mb-1">
              Manage your team requirements and find the perfect members
            </p>
            <div className="flex items-center text-white/80 text-lg mt-2">
          ...
          <div className="flex items-center gap-4">
            <div className="text-white/90 text-sm">
              <div className="font-medium text-right">{requirements.length} requirement{requirements.length !== 1 ? "s" : ""}</div>
              <div className="text-xs text-white/80">Manage & find members</div>
            </div>
    ...
    <>
      <div className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border-0 relative">
        <div className="p-6">
          ...
          {/* Team Name */}
          <h3 className="text-xl font-bold text-gray-900 mb-2">{requirement.teamname || requirement.teamName || "Team Name"}</h3>
          ...
          {requirement.domains && (
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {requirement.domains.split(",").map((domain, idx) => (
